...
# Metro Train Predictions App - Fixed Installation Script
# Improved health check timing for MySQL initialization
...
# Enhanced service health validation with longer timeout for MySQL
wait_for_service_health() {
    local service_name=$1
    local max_attempts=60  # Increased from 30 to 60 for MySQL init
    local attempt=1
    ...
    while [ $attempt -le $max_attempts ]; do
        ...
        case $health_status in
            ...
            "unhealthy")
                ...
                ;;
            "starting"|"unknown")
                if [ "$service_name" = "mysql" ] && [ $attempt -gt 40 ]; then
                    # For MySQL, test actual connectivity after 40 attempts
                    ...
                    if docker compose exec mysql mysql -u root -ppassword -e "SELECT 1;" >/dev/null 2>&1; then
                        log "MySQL is responding to queries (may still be initializing health check)"
                        return 0
                    fi
                else
    ...
    # Final connectivity test for MySQL
...
validate_api_endpoint() {
    local max_attempts=15  # Increased attempts
    local attempt=1
...
main() {
    ...
    # Wait for and validate each service with improved timing
    log "Validating service health (this may take up to 2 minutes for MySQL initialization)..."
